...
def detect_ball(image):
    ...
    lower_orange = np.array([10, 100, 100])  # Adjust these values based on the ball's actual color
    upper_orange = np.array([25, 255, 255])  # Adjust these values based on the ball's actual color
...
def predict_trajectory(ball_pos, ball_velocity, hoop_z, g=9.81):
    ...
    return (X_pred, Y_pred), time_to_hoop

def adjust_hoop_position(sim, hoop_handle, target_position, current_position, max_speed=1.0):
    """
...
def control_logic(sim):
    ...
    try:
        ...
        print('Left Vision Sensor Handle:', left_vision_sensor_handle)

        right_vision_sensor_handle = sim.getObject(r'/basket_bot/cam_r')
        print('Right Vision Sensor Handle:', right_vision_sensor_handle)

        hoop_handle = sim.getObject(r'/basket_bot/hoop_odom')
        print('Hoop Handle:', hoop_handle)
        print(hoop_position(sim, hoop_handle))
        ...
        while True:
            try:
                ...
                if left_img is not None and right_img is not None:
                    # Detect ball in the left and right images
                    ...
                    if ball_left and ball_right:
                        ...
                        ball_pos_3d = calculate_3d_position(
                            ball_left, ball_right, focal_length, baseline, resolution
                        )
                        ...
                        # TODO: Add hoop movement logic here (e.g., PID controller)
                ...
                print(f"Error in the control loop: {loop_error}")
                break
    except Exception as e:
